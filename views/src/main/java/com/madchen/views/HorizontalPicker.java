...
public class HorizontalPicker extends View {
    ...
    private void selectItem() {
        ...
        if (onItemClicked != null) {
            post(new Runnable() {
                @Override
                public void run() {
                    onItemClicked.onItemClicked(getSelectedItemIndex());
                }
            });
        }
    ...
    public String getSelectedItem() {
        return this.values[getSelectedItemIndex()].toString();
    }
...
//            postInvalidateOnAnimation(); // TODO
     ...
     * {@link com.madchen.views.HorizontalPicker#getInBoundsX(int)}
     *
     * @param x Relative scroll position to calculate
     * @return Current scroll position + {param x} if is within our scroll bounds, otherwise it will
     * return min/max scroll position.
     */
